...
2. Installs dependencies with `npm ci` and builds the site with `npm run build` (Eleventy, then a postbuild step).
   `src/index.html` and `src/en/index.html` are wrapped in the shared layout at `src/_includes/base.njk`.
   Everything else under `src/` (the tool directories and `src/static/`) is copied into `site/` unchanged, never templated - except that `scripts/inject-analytics.js` then inserts the same PostHog snippet (`src/_includes/analytics.njk`) before `</head>` in each of those pages too, so every page is tracked without editing vendored markup by hand.
3. Runs `npx html-validate@11` over the built `site/index.html` and `site/en/index.html`.
...
4. Substitutes the `POSTHOG_PROJECT_KEY` secret into the built `site/index.html` and `site/en/index.html`.
5. Uploads `site/` to `~/michalkulbacki.com/public_html/` on the server with `rsync -az --delete` over SSH.

`site/` is generated by the build and is not committed to the repository.
...
Build first, since `site/` doesn't exist until you do:
